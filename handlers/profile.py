...
async def handle_gender(message: Message, state: FSMContext):
    ...
    text = """📋 Bu NIKOH platformasi. 
    
ℹ️ Anketani faqat valiyingiz (ota-ona, aka-uka) roziligi bilan to'ldiring.
...
async def handle_height(message: Message, state: FSMContext):
    ...
    await message.answer("⚖️ Vazningizni kiriting (30-200 kg):")
...
async def handle_nationality(message: Message, state: FSMContext):
    ...
    if message.text == "Boshqa":
        await message.answer("✏️ Millatingizni kiriting:")
...
async def handle_hijab(message: Message, state: FSMContext):
    ...
    await message.answer("✈️ Ko'chib o'tishga tayyormisiz?", reply_markup=yes_no_kb())
...
async def handle_country(message: Message, state: FSMContext):
    ...
    if message.text == "O'zbekiston":
        await message.answer("🏙️ Viloyatni tanlang:", reply_markup=regions_kb())
...
async def handle_prays(message: Message, state: FSMContext):
    ...
    await message.answer("🗣️ Nechta til bilasiz? (1-10):")
...
async def handle_languages(message: Message, state: FSMContext):
    ...
    if not languages:
        await message.answer("⚠️ Iltimos, 1-10 oralig'ida bo'lgan til sonini kiriting:")
        return
...
ℹ️ **Eslatma:** Nomzodlar sizga yoza olishi uchun sozlamalaringizni shunday ko'rinishga keltiring Sozlamalar -> Foydalanuvchi nomi -> zuhra1608 (hohlagan nom qo'yasiz) (Agar qanday qilishni tushunmagan bo'lsangiz, ushbu rasmga qarang: https://t.me/nikohboti/8)"""
    
    await callback.message.answer(text)
    
    text2 = """✅ Profilingiz saqlandi va maxfiy anketalar bo'limiga qo'shildi! 

🔒 Hozirda profilingizni faqat VIP A'zolar ko'ra oladi. 

📢 Agar @Nikoh_01 kanaliga e'lon berishni istasangiz, Bosh menyudan "E'lon joylashtirish" tugmasi orqali e'lon bera olasiz (@nikohboti kanalida video qilib ko'rsatilgan)"""
